...
namespace Index
{
    ...
    float IndexPhysicsEngine::s_UpdateTimestep = 1.0f / 60.0f;

    IndexPhysicsEngine::IndexPhysicsEngine()
        : m_IsPaused(true)
        , m_UpdateAccum(0.0f)
    ...
    void IndexPhysicsEngine::OnUpdate(const TimeStep& timeStep, Scene* scene)
    {
        ...
        if(!m_IsPaused)
        {
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::Get Rigid Bodies");
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::Get Spring Constraints");
                m_Constraints.clear();
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::Get Axis Constraints");
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::Get Distance Constraints");
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::Get Weld Constraints");
            ...
            {
                INDEX_PROFILE_SCOPE("Physics::UpdatePhysics");
                ...
                {
                    INDEX_LOG_WARN("Physics too slow to run in real time!");
    ...
    void IndexPhysicsEngine::UpdatePhysics()
    {
        m_Manifolds.clear();
    ...
    void IndexPhysicsEngine::UpdateRigidBodys()
    {
        INDEX_PROFILE_SCOPE("Update Rigid Body");
        for(size_t i = 0; i < m_RigidBodys.size(); i++)
            UpdateRigidBody(m_RigidBodys[i]);
    }

    void IndexPhysicsEngine::SyncTransforms(Scene* scene)
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void IndexPhysicsEngine::UpdateRigidBody(RigidBody3D* obj) const
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void IndexPhysicsEngine::BroadPhaseCollisions()
    {
        INDEX_PROFILE_FUNCTION();
        ...
        INDEX_LOG_INFO(duplicatePairs);
#endif
    }

    void IndexPhysicsEngine::NarrowPhaseCollisions()
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void IndexPhysicsEngine::SolveConstraints()
    {
        INDEX_PROFILE_FUNCTION();

        {
            INDEX_PROFILE_SCOPE("Solve Manifolds");

            for(Manifold& m : m_Manifolds)
                m.PreSolverStep(s_UpdateTimestep);
        }
        {
            INDEX_PROFILE_SCOPE("Solve Constraints");
        ...
        {
            INDEX_PROFILE_SCOPE("Apply Impulses");
    ...
    void IndexPhysicsEngine::ClearConstraints()
    {
        m_Constraints.clear();
    }

    std::string IndexPhysicsEngine::IntegrationTypeToString(IntegrationType type)
    {
        switch(type)
        {
        case IntegrationType::EXPLICIT_EULER:
    ...
    std::string IndexPhysicsEngine::BroadphaseTypeToString(BroadphaseType type)
    {
        switch(type)
        {
    ...
    void IndexPhysicsEngine::SetBroadphaseType(BroadphaseType type)
    {
        ...
        {
        case BroadphaseType::BRUTE_FORCE:
            m_BroadphaseDetection = Index::CreateSharedPtr<BruteForceBroadphase>();
            break;
        case BroadphaseType::SORT_AND_SWEAP:
            m_BroadphaseDetection = Index::CreateSharedPtr<SortAndSweepBroadphase>();
            break;
        case BroadphaseType::OCTREE:
            m_BroadphaseDetection = Index::CreateSharedPtr<OctreeBroadphase>(5, 5, Index::CreateSharedPtr<BruteForceBroadphase>());
            break;
        default:
            m_BroadphaseDetection = Index::CreateSharedPtr<BruteForceBroadphase>();
            break;
    ...
    void IndexPhysicsEngine::OnImGui()
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void IndexPhysicsEngine::OnDebugDraw()
    {
        INDEX_PROFILE_FUNCTION();
