...
namespace Index
{
        ...
        , m_SceneViewHeight(600)
    {
        ...
        INDEX_ASSERT(!s_Instance, "Application already exists!");
        s_Instance = this;
    }
    ...
    void Application::OpenNewProject(const std::string& path, const std::string& name)
    {
        ...
        m_ProjectSettings.Width       = 1080;
        m_ProjectSettings.Height      = 696;
        ...
        // This is assuming Application in bin/Release-macos-x86_64/IndexEditor.app
        INDEX_LOG_INFO(StringUtilities::GetFileLocation(OS::Instance()->GetExecutablePath()));
        m_ProjectSettings.m_EngineAssetPath = StringUtilities::GetFileLocation(OS::Instance()->GetExecutablePath()) + "../../../../../Index/Assets/";
#else
        m_ProjectSettings.m_EngineAssetPath = StringUtilities::GetFileLocation(OS::Instance()->GetExecutablePath()) + "../../Index/Assets/";
#endif
        ...
        m_SceneManager->EnqueueScene(new Scene("Untitled"));
    ...
    void Application::Init()
    {
        ...
        System::JobSystem::Execute(context, [this](JobDispatchArgs args)
                                   {
                                       m_SystemManager->RegisterSystem<IndexPhysicsEngine>();
        ...
        INDEX_LOG_INFO("Initialised ImGui Manager");
    ...
    bool Application::OnFrame()
    {
        ...
        {
            INDEX_PROFILE_SCOPE("Application::SceneSwitch");
        ...
        if(ts.GetSeconds() > 5)
        {
            INDEX_LOG_WARN("Large frame time {0}", ts.GetSeconds());
#ifdef INDEX_DISABLE_LARGE_FRAME_TIME
        ...
        {
            INDEX_PROFILE_SCOPE("Application::TimeStepUpdates");
            ts.OnUpdate();
        ...
        {
            INDEX_PROFILE_SCOPE("Application::ImGui::NewFrame");
            ImGui::NewFrame();
        }

        {
            INDEX_PROFILE_SCOPE("Application::Update");
            OnUpdate(ts);

            {
                // INDEX_PROFILE_SCOPE("Wait System update");
                // updateThread.join();
                UpdateSystems();

                m_SystemManager->GetSystem<IndexPhysicsEngine>()->SyncTransforms(m_SceneManager->GetCurrentScene());
        ...
        if(!m_Minimized)
        {
            INDEX_PROFILE_SCOPE("Application::Render");
        ...
        {
            INDEX_PROFILE_SCOPE("Application::UpdateGraphicsStats");
        ...
        {
            INDEX_PROFILE_SCOPE("Application::WindowUpdate");
        ...
        {
            INDEX_PROFILE_SCOPE("Application::FrameRateCalc");
            m_SecondTimer += 1.0f;
    ...
    void Application::Serialise()
    {
        INDEX_PROFILE_FUNCTION();
        {
            ...
            auto fullPath = m_ProjectSettings.m_ProjectRoot + m_ProjectSettings.m_ProjectName + std::string(".IndexProject");
            INDEX_LOG_INFO("Serialising Application {0}", fullPath);
    ...
    void Application::Deserialise()
    {
        INDEX_PROFILE_FUNCTION();
        {
            auto filePath = m_ProjectSettings.m_ProjectRoot + m_ProjectSettings.m_ProjectName + std::string(".IndexProject");

            MountVFSPaths();

            if(!FileSystem::FileExists(filePath))
            {
                INDEX_LOG_INFO("No saved Project file found {0}", filePath);
                {
                    m_SceneManager = CreateUniquePtr<SceneManager>();
            ...
            catch(...)
            {
                ...
                INDEX_LOG_ERROR("Failed to load project");
            }
        }
    }
}
