...
namespace Index
{
    ...
    public:
        ...
        inline Reference(const Reference<U>& moving) noexcept
        {
            ...
            {
                INDEX_LOG_ERROR("Failed to cast Reference");
            }
        }
    ...
    private:
        ...
        {
            INDEX_ASSERT(ptr, "Creating shared ptr with nullptr");
    ...
    template <class T>
    class INDEX_EXPORT WeakReference
    {
    public:
        ...
        explicit WeakReference(T* ptr) noexcept
            : m_Ptr(ptr)
        {
            INDEX_ASSERT(ptr, "Creating weak ptr with nullptr");
...
{
    template <typename T>
    struct hash<Index::Reference<T>>
    {
        size_t operator()(const Index::Reference<T>& x) const
        {
            return hash<T*>()(x.get());
        }
    };
