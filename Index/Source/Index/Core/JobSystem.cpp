    ...
    namespace System
    {
        ...
        namespace JobSystem
        {
            ...
            void OnInit(uint32_t maxThreadCount)
            {
                ...
                internal_state->numThreads = Index::Maths::Min(maxThreadCount, Index::Maths::Max(1u, internal_state->numCores - 1));
                ...
                for(uint32_t threadID = 0; threadID < internal_state->numThreads; ++threadID)
                {
                    std::thread& worker = internal_state->threads.EmplaceBack([threadID]
                                ...
                                while (internal_state->alive.load())
                                {
                                    ...
                                    INDEX_PROFILE_SETTHREADNAME(ss.str().c_str());
...
#define handle_error_en(en, msg) \
    do {                         \
        ...
        perror(msg);             \
                    ...
                    INDEX_PROFILE_SETTHREADNAME(wss.str().c_str());
#endif

                    worker.detach();
                }

                INDEX_LOG_INFO("Initialised JobSystem with [{0} cores] [{1} threads]", internal_state->numCores, internal_state->numThreads);
            }
