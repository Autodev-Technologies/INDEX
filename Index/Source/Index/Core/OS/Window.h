...
namespace Index
{
    struct INDEX_EXPORT WindowDesc
    {
        WindowDesc(uint32_t width = 1080, uint32_t height = 696, int renderAPI = 0, const std::string& title = "App", bool fullscreen = false, bool vSync = true, bool borderless = false, const std::string& filepath = "")
            : Width(width)
            , Height(height)
            , Title(title)
    ...
    public:
        ...
        const SharedPtr<Index::Graphics::SwapChain>& GetSwapChain() const { return m_SwapChain; }
        const SharedPtr<Index::Graphics::GraphicsContext>& GetGraphicsContext() const { return m_GraphicsContext; }
    ...
    protected:
        ...
        SharedPtr<Index::Graphics::SwapChain> m_SwapChain;
        SharedPtr<Index::Graphics::GraphicsContext> m_GraphicsContext;
    };
