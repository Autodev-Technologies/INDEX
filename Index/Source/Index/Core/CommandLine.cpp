...
namespace Index
{
	...
	{
        if(!m_Enabled) 
			INDEX_LOG_ERROR("Option {0} is not provided. Type --help or -h for help", m_LongName);
        
        if(m_ArgValue.empty())
			INDEX_LOG_ERROR("Option {0} does not have an argument. Type --help or -h for help", m_LongName);
    }
    ...
    {
        m_Description = "Index Command Line Parser";
    }
    ...
    void CommandLine::Parse(int argc, char** argv)
    {
        ...
        while(i < argc)
        {
            ...
            if(!foundArgument)
            {
                INDEX_LOG_ERROR("Ignoring unknown command line argument {0}", flag);
