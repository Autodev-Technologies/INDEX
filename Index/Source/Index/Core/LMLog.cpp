...
namespace Index::Debug
{
    ...
    void Log::OnInit()
    {
        ...
        auto logFileSink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>("IndexLog.txt", 1048576 * 5, 3);
        sinks.emplace_back(logFileSink); // Log file
#endif

        // create the loggers
        s_CoreLogger = std::make_shared<spdlog::logger>("Index", begin(sinks), end(sinks));
