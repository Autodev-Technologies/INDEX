...
namespace Index
{
    AudioData LoadOgg(const std::string& fileName)
    {
        ...
        if(!Index::VFS::Get().ResolvePhysicalPath(fileName, physicalPath))
        {
            INDEX_LOG_INFO("Failed to load Ogg file : File Not Found");
        }
        ...
        if(!m_FileHandle)
        {
            INDEX_LOG_CRITICAL("Failed to load OGG file '{0}'!", physicalPath);
        }
        int error;
        ...
        if(!m_StreamHandle)
        {
            INDEX_LOG_CRITICAL("Failed to load OGG file '{0}'! , Error {1}", physicalPath, error);
            return data;
        }
