...
namespace Index::Graphics
{
        ...
        for(tinygltf::Texture& gltfTexture : gltfModel.textures)
        {
            ...
            if(imageAndSampler.Image)
            {
                ...
                if(gltfTexture.sampler != -1)
                    ...
                    INDEX_LOG_WARN("MISSING SAMPLER");
    ...
    std::vector<Graphics::Mesh*> LoadMesh(tinygltf::Model& model, tinygltf::Mesh& mesh, std::vector<SharedPtr<Material>>& materials, Maths::Transform& parentTransform)
    {
        ...
        for(auto& primitive : mesh.primitives)
        {
            ...
            // -------- Indices ----------
            {
                ...
                {
                    INDEX_LOG_WARN("Unsupported indices data type - {0}", componentTypeByteSize);
                }
            }
    ...
    void LoadNode(Model* mainModel, int nodeIndex, const glm::mat4& parentTransform, tinygltf::Model& model, std::vector<SharedPtr<Material>>& materials, std::vector<std::vector<Graphics::Mesh*>>& meshes)
    {
        ...
        INDEX_LOG_INFO("asset.copyright : {0}", model.asset.copyright);
        INDEX_LOG_INFO("asset.generator : {0}", model.asset.generator);
        INDEX_LOG_INFO("asset.version : {0}", model.asset.version);
        INDEX_LOG_INFO("asset.minVersion : {0}", model.asset.minVersion);
#endif
    ...
    void Model::LoadGLTF(const std::string& path)
    {
        ...
        if(!warn.empty())
        {
            INDEX_LOG_ERROR(warn);
        }

        if(!ret)
        {
            INDEX_LOG_ERROR("Failed to parse glTF");
        }
        {
            INDEX_PROFILE_SCOPE("Parse GLTF Model");
            ...
            const tinygltf::Scene& gltfScene = model.scenes[Index::Maths::Max(0, model.defaultScene)];
            ...
            if(!skins.empty())
            {
                ...
                for(auto& animName : animationNames)
                {
                    ...
                    INDEX_LOG_INFO("Loaded Anim : {0}", animName);
                }
            }
        }
    }
