...
namespace Index::Graphics
{
    ...
    glm::vec2 ToIndexVector(const ofbx::Vec2& vec)
    {
        return glm::vec2(float(vec.x), float(vec.y));
    }

    glm::vec3 ToIndexVector(const ofbx::Vec3& vec)
    {
        return glm::vec3(float(vec.x), float(vec.y), float(vec.z));
    }

    glm::vec4 ToIndexVector(const ofbx::Vec4& vec)
    {
        return glm::vec4(float(vec.x), float(vec.y), float(vec.z), float(vec.w));
    }

    glm::vec4 ToIndexVector(const ofbx::Color& vec)
    {
        return glm::vec4(float(vec.r), float(vec.g), float(vec.b), 1.0f);
    }

    glm::quat ToIndexQuat(const ofbx::Quat& quat)
    {
    ...
    SharedPtr<Material> LoadMaterial(const ofbx::Material* material, bool animated)
    {
        ...
        properties.albedoColour = ToIndexVector(material->getDiffuseColor());
    ...
    void Model::LoadFBX(const std::string& path)
    {
        ...
        if(data == nullptr)
        {
            INDEX_LOG_WARN("Failed to load fbx file");
            return;
        }
