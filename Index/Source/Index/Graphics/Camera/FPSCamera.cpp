...
namespace Index
{
    ...
    void FPSCameraController::HandleKeyboard(Maths::Transform& transform, float dt)
    {
        ...
        if(Input::Get().GetKeyHeld(Index::InputCode::Key::W))
        {
            m_Velocity += transform.GetForwardDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Index::InputCode::Key::S))
        {
            m_Velocity -= transform.GetForwardDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Index::InputCode::Key::A))
        {
            m_Velocity -= transform.GetRightDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Index::InputCode::Key::D))
        {
            m_Velocity += transform.GetRightDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Index::InputCode::Key::Space))
        {
            m_Velocity -= transform.GetUpDirection() * m_CameraSpeed;
        }

        if(Input::Get().GetKeyHeld(Index::InputCode::Key::LeftShift))
        {
            m_Velocity += transform.GetUpDirection() * m_CameraSpeed;
        }
