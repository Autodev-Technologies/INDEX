...
#define MAX_VALUE_NAME 8192
        ...
        void RenderDoc::OnPreDeviceCreation()
        {
            ...
            if(rdc_api == nullptr)
            {
                ...
                if(rdc_module == nullptr)
                {
                    ...
                    INDEX_ASSERT(!RDocDllPaths.empty(), "Could not find any install locations for renderdoc.dll");
                    std::wstring module_path = RDocDllPaths[0]; // assuming x64 is reported first
                    rdc_module               = ::LoadLibraryW(module_path.c_str());
                }

                INDEX_ASSERT(rdc_module != nullptr, "Failed to get RenderDoc module");
                ...
                UNIMPLEMENTED;
#endif
                INDEX_ASSERT(rdc_get_api != nullptr, "Failed to RENDERDOC_GetAPI function address from renderdoc.dll");

                int result = rdc_get_api(eRENDERDOC_API_Version_1_5_0, (void**)&rdc_api);
                INDEX_ASSERT(result != 0, "Failed to get RenderDoc API pointer");
            }

            INDEX_ASSERT(rdc_api != nullptr, "RenderDoc API has not been initialised");
        ...
        void RenderDoc::FrameCapture()
        {
            ...
            {
                INDEX_LOG_INFO("Bringing RenderDoc to foreground...");
                rdc_api->ShowReplayUI();
                return;
            }

            // If the RenderDoc UI is not running, launch it and connect.
            INDEX_LOG_INFO("Launching RenderDoc...");
            if(rdc_api->LaunchReplayUI(true, "") == 0)
            {
                INDEX_LOG_ERROR("Failed to launch RenderDoc");
            }
        }
    }
}
