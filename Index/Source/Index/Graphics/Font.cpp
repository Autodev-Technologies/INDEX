...
#if FONT_DEBUG_LOG
#define FONT_LOG(...) INDEX_LOG_INFO("Font", __VA_ARGS__)
#else
#define FONT_LOG(...)
#endif
...
#define THREADS 8
        ...
        static void CacheFontAtlas(const std::string& fontName, float fontSize, AtlasHeader header, const void* pixels)
        {
            ...
            {
                stream.close();
                INDEX_LOG_ERROR("Failed to cache font atlas to {0}", filepath.string());
                return;
            }
        ...
        void Font::Init()
        {
            ...
            {
                INDEX_LOG_ERROR("Font: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
            }
            atlasPacker.getDimensions(config.width, config.height);
            INDEX_CORE_ASSERT(config.width > 0 && config.height > 0, "");
