...
namespace Index
{
    namespace Graphics
    {
        ...
        VKPhysicalDevice::VKPhysicalDevice()
        {
            ...
            if(m_GPUCount == 0)
            {
                INDEX_LOG_CRITICAL("No GPUs found!");
            }
            ...
            if(desiredGPUIndex >= 0)
            {
                ...
                {
                    INDEX_LOG_CRITICAL("GPU index greater than GPU count!");
                }
                else
                {
            ...
            INDEX_LOG_INFO("Vulkan : {0}.{1}.{2}", VK_API_VERSION_MAJOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_MINOR(m_PhysicalDeviceProperties.apiVersion), VK_API_VERSION_PATCH(m_PhysicalDeviceProperties.apiVersion));
            INDEX_LOG_INFO("GPU : {0}", m_DeviceInfo.Name);
            INDEX_LOG_INFO("Memory : {0} mb", StringUtilities::ToString(m_DeviceInfo.Memory));
            INDEX_LOG_INFO("Vendor : {0}", m_DeviceInfo.Vendor);
            INDEX_LOG_INFO("Vendor ID : {0}", StringUtilities::ToString(m_DeviceInfo.VendorID));
            INDEX_LOG_INFO("Device Type : {0}", std::string(PhysicalDeviceTypeToString(m_DeviceInfo.Type)));
            INDEX_LOG_INFO("Driver Version : {0}", m_DeviceInfo.Driver);
            INDEX_LOG_INFO("APi Version : {0}", m_DeviceInfo.APIVersion);
            ...
            INDEX_ASSERT(queueFamilyCount > 0, "");
            ...
            if(extCount > 0)
            {
                ...
                {
                    INDEX_LOG_INFO("Selected physical device has {0} extensions", extensions.size());
        ...
        uint32_t VKPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
        {
            ...
            INDEX_ASSERT(false, "Could not find a suitable memory type!");
            return UINT32_MAX;
        }
...
#endif
#if defined(INDEX_PROFILE) && defined(TRACY_ENABLE)
            for(int i = 0; i < 3; i++)
        ...
        bool VKDevice::Init()
        {
            ...
            {
                INDEX_LOG_CRITICAL("[VULKAN] vkCreateDevice() failed!");
                return false;
            }
            ...
            {
                INDEX_LOG_CRITICAL("[VULKAN] Failed to create VMA allocator");
            }
#endif
