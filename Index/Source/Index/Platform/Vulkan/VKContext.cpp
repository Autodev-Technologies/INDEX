...
namespace Index
{
    namespace Graphics
    {
        ...
        const std::vector<const char*> VKContext::GetRequiredExtensions()
        {
            ...
            {
                INDEX_LOG_INFO("Vulkan : Enabled Validation Layers");
...
#if 0
#if defined(TRACY_ENABLE) && defined(INDEX_PLATFORM_WINDOWS)
                                          ...
                                          void* userData)
        {
            ...
            {
                INDEX_LOG_WARN("[VULKAN] - ERROR : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Warnings may hint at unexpected / non-spec API usage
            if(flags & VK_DEBUG_REPORT_WARNING_BIT_EXT)
            {
                INDEX_LOG_WARN("[VULKAN] - WARNING : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // May indicate sub-optimal usage of the API
            if(flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT)
            {
                INDEX_LOG_WARN("[VULKAN] - PERFORMANCE : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Informal messages that may become handy during debugging
            if(flags & VK_DEBUG_REPORT_INFORMATION_BIT_EXT)
            {
                INDEX_LOG_WARN("[VULKAN] - INFO : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            // Diagnostic info from the Vulkan loader and layers
            // Usually not helpful in terms of API usage, but may help to debug layer and loader problems
            if(flags & VK_DEBUG_REPORT_DEBUG_BIT_EXT)
            {
                INDEX_LOG_INFO("[VULKAN] - DEBUG : [{0}] Code {1}  : {2}", pLayerPrefix, msgCode, pMsg);
            }
            ...
            validationLayers.erase(
                std::remove_if(
                    ...
                    [&](const char* layerName)
                    {
                        ...
                        {
                            removedLayer = true;
                            INDEX_LOG_WARN("[VULKAN] Layer not supported - {0}", layerName);
                        }
                ...
                std::remove_if(
                    ...
                    [&](const char* extensionName)
                    {
                        ...
                        if(!extensionFound)
                        {
                            ...
                            INDEX_LOG_WARN("[VULKAN] Extension not supported - {0}", extensionName);
                        }
        ...
        void VKContext::CreateInstance()
        {
            ...
            {
                INDEX_LOG_CRITICAL("volkInitialize failed");
            }

            if(volkGetInstanceVersion() == 0)
            {
                INDEX_LOG_CRITICAL("Could not find loader");
            }
#endif
            ...
            {
                INDEX_LOG_WARN("[VULKAN] One or multiple Validation layers requested are not available!");
            }

            if(!CheckExtensionSupport(m_InstanceExtensionNames))
            {
                INDEX_LOG_WARN("[VULKAN] One or multiple Extensions requested are not available!");
            }
            ...
            if(sdkVersion > driverVersion)
            {
                ...
                // INDEX_LOG_WARN("Using Vulkan {0}. Please update your graphics drivers to support Vulkan {1}.", driverVersionStr, sdkVersionStr);
            }

            appInfo.sType              = VK_STRUCTURE_TYPE_APPLICATION_INFO;
            appInfo.pApplicationName   = "Runtime";
            appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
            appInfo.pEngineName        = "Index";
            appInfo.engineVersion      = VK_MAKE_VERSION(IndexVersion.major, IndexVersion.minor, IndexVersion.patch);
            ...
            if(createResult != VK_SUCCESS)
            {
                ...
                INDEX_LOG_CRITICAL("[VULKAN] Failed to create instance!");
            }
#ifndef INDEX_PLATFORM_IOS
        ...
        void VKContext::SetupDebugCallback()
        {
            ...
            {
                INDEX_LOG_CRITICAL("[VULKAN] Failed to set up debug callback!");
            }
        }
