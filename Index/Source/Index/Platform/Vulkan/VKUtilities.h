...
#define VK_CHECK_RESULT(f)                                                                                                                        \
    {                                                                                                                                             \
        ...
        if(res != VK_SUCCESS)                                                                                                                     \
        {                                                                                                                                         \
            INDEX_LOG_ERROR("[VULKAN] : VkResult is {0} in {1} at line {2}", Index::Graphics::VKUtilities::ErrorString(res), __FILE__, __LINE__); \
        }                                                                                                                                         \
    }
...
namespace Index
{
    namespace Graphics
    {
        namespace VKUtilities
        {
            ...
            VkPolygonMode PolygonModeToVk(Index::Graphics::PolygonMode mode);
            VkPrimitiveTopology DrawTypeToVk(Index::Graphics::DrawType type);
        }
    }
}
