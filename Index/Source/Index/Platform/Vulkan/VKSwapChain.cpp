...
namespace Index
{
    namespace Graphics
    {
        ...
        bool VKSwapChain::Init(bool vsync)
        {
            ...
            if(!m_Surface)
            {
                INDEX_LOG_CRITICAL("[VULKAN] Failed to create window surface!");
            }
            ...
            if(queueIndexSupported == VK_FALSE)
                INDEX_LOG_ERROR("Present Queue not supported");
        ...
        {
            INDEX_ASSERT(m_CurrentBuffer < m_SwapChainBufferCount, "Incorrect swapchain buffer index");
        ...
        void VKSwapChain::AcquireNextImage()
        {
            ...
            {
                INDEX_PROFILE_SCOPE("vkAcquireNextImageKHR");
                ...
                {
                    INDEX_LOG_INFO("Acquire Image result : {0}", result == VK_ERROR_OUT_OF_DATE_KHR ? "Out of Date" : "SubOptimal");
                ...
                {
                    FailedCount++;
                    INDEX_LOG_CRITICAL("[VULKAN] Failed to acquire swap chain image! - {0}", VKUtilities::ErrorString(result));

                    if(FailedCount > 10)
                    {
                        INDEX_LOG_CRITICAL("[VULKAN] Failed to acquire swap chain image {0} times! - Exiting", FailedCount);
        ...
        void VKSwapChain::Present(VkSemaphore semaphore)
        {
            ...
            {
                INDEX_LOG_ERROR("[Vulkan] SwapChain out of date");
            }
            else if(error == VK_SUBOPTIMAL_KHR)
            {
                INDEX_LOG_ERROR("[Vulkan] SwapChain suboptimal");
            }
            else
            {
