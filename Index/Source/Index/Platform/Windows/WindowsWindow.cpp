...
#define HID_USAGE_GENERIC_KEYBOARD ((USHORT)0x06)
    ...
    static HICON createIcon(unsigned char* image, int width, int height, int xhot, int yhot, bool icon)
    {
        ...
        if(!color)
        {
            INDEX_LOG_ERROR("Win32: Failed to create RGBA bitmap");
            return NULL;
        }
        ...
        if(!mask)
        {
            INDEX_LOG_ERROR("Win32: Failed to create mask bitmap");
            DeleteObject(color);
            return NULL;
        }
        ...
        if(!handle)
        {
            if(icon)
            {
                INDEX_LOG_ERROR("Win32: Failed to create icon");
            }
            else
            {
                INDEX_LOG_ERROR("Win32: Failed to create cursor");
            }
        }
    ...
    bool WindowsWindow::Init(const WindowDesc& properties)
    {
        ...
        {
            INDEX_LOG_CRITICAL("Could not register Win32 class!");
            return false;
        }
        ...
        if(!hWnd)
        {
            INDEX_LOG_CRITICAL("Could not create window!");
            return false;
        }
        ...
        if(pixelFormat)
        {
            ...
            {
                INDEX_LOG_CRITICAL("Failed setting pixel format!");
                return false;
            }
        }
        else
        {
            INDEX_LOG_CRITICAL("Failed choosing pixel format!");
            return false;
        }
		...
		if(properties.RenderAPI == RenderAPI::OpenGL)
		{
            ...
            if(!gladLoadWGL(hDc))
                INDEX_LOG_ERROR("glad failed to load WGL!");
            if(!gladLoadGL())
                INDEX_LOG_ERROR("glad failed to load OpenGL!");
            ...
            if(hrc == nullptr)
            {
                INDEX_LOG_ERROR("Failed to create core OpenGL context");
            }
            else
            {
    ...
    void MouseButtonCallback(Window* window, int32_t button, int32_t x, int32_t y)
    {
        ...
        bool down = false;
        Index::InputCode::MouseKey mouseKey;
        switch(button)
        {
        case WM_LBUTTONDOWN:
            SetCapture(hWnd);
            mouseKey = Index::InputCode::MouseKey::ButtonLeft;
            down     = true;
            break;
        case WM_LBUTTONUP:
            ReleaseCapture();
        ...
        case WM_RBUTTONDOWN:
            SetCapture(hWnd);
            mouseKey = Index::InputCode::MouseKey::ButtonRight;
            down     = true;
            break;
        case WM_RBUTTONUP:
            ReleaseCapture();
        ...
        case WM_MBUTTONDOWN:
            SetCapture(hWnd);
            mouseKey = Index::InputCode::MouseKey::ButtonMiddle;
            down     = true;
            break;
        case WM_MBUTTONUP:
            ReleaseCapture();
    ...
    void CharCallback(Window* window, int32_t key, int32_t flags, UINT message)
    {
        ...
        KeyTypedEvent event(Index::InputCode::Key {}, char(key));
        data.EventCallback(event);
    }
    ...
    void KeyCallback(Window* window, int32_t key, int32_t flags, UINT message)
    {
        ...
        if(pressed)
        {
            KeyPressedEvent event(WindowsKeyCodes::WindowsKeyToIndex(key), repeat ? 1 : 0);
            data.EventCallback(event);
        }
        else
        {
            KeyReleasedEvent event(WindowsKeyCodes::WindowsKeyToIndex(key));
            data.EventCallback(event);
        }
    }
    ...
    void WindowsWindow::SetIcon(const std::string& filePath, const std::string& smallIconFilePath)
    {
        ...
        if(filePath != "")
        {
            ...
            uint8_t* pixels = Index::LoadImageFromFile(filePath, &width, &height, nullptr, nullptr, true);
        ...
        {
            uint32_t width, height;
            uint8_t* pixels = Index::LoadImageFromFile(smallIconFilePath, &width, &height, nullptr, nullptr, true);
