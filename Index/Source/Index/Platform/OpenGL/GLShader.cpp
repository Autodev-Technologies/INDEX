...
namespace Index
{
    namespace Graphics
    {
        ...
        GLShader::GLShader(const uint32_t* vertData, uint32_t vertDataSize, const uint32_t* fragData, uint32_t fragDataSize)
        {
            ...
            if(!m_Handle)
            {
                INDEX_LOG_ERROR("{0} - {1}", error.message[error.shader], m_Name);
            }
            else
            {
                INDEX_LOG_INFO("Successfully compiled shader: {0}", m_Name);
            }
        ...
        void GLShader::SetUniform(ShaderDataType type, uint8_t* data, uint32_t size, uint32_t offset, const std::string& name)
        {
            ...
            if(m_UniformLocations.find(hashName) == m_UniformLocations.end())
            {
                ...
                {
                    INDEX_LOG_WARN("Invalid uniform location {0}", name);
                }
            }
            ...
            if(location == -1)
            {
                INDEX_LOG_ERROR("Couldnt Find Uniform In Shader: {0}", name);
                return;
            }
            ...
            case ShaderDataType::MAT4ARRAY:
                ...
                break;
            default:
                INDEX_ASSERT(false, "Unknown type!");
            }
        }
        ...
        bool GLShader::SetUniformLocation(const std::string& szName, bool pc)
        {
            ...
            INDEX_LOG_WARN("Couldnt set uniform location");
            return false;
        }
            ...
            for(uint32_t i = 0; i < lines.size(); i++)
            {
                ...
                else if(StringUtilities::StartsWith(str, "#include"))
                {
                    ...
                    if(strstr(file.c_str(), rem.c_str()))
                    {
                        ...
                        INDEX_LOG_WARN("Including file \'{0}\' into shader.", file);
        ...
        uint32_t GLShader::Compile(std::map<ShaderType, std::string>* sources, GLShaderErrorInfo& info)
        {
            ...
            if(result == GL_FALSE)
            {
                ...
                INDEX_LOG_ERROR(info.message[info.shader]);
                return 0;
            }
...
#endif
            default:
                INDEX_LOG_ERROR("Unsupported Shader Type");
                return GL_VERTEX_SHADER;
            }
        }
