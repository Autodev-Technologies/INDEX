...
namespace Index
{
    namespace Graphics
    {
        ...
        uint8_t* GLTexture2D::LoadTextureData()
        {
            ...
            {
                uint32_t bits;
                pixels              = Index::LoadImageFromFile(m_FileName.c_str(), &m_Width, &m_Height, &bits, &isHDR, !m_LoadOptions.flipY);
        ...
        uint32_t GLTextureCube::LoadFromMultipleFiles()
        {
            ...
            bool isHDR  = false;
            uint8_t* xp = Index::LoadImageFromFile(xpos, &width, &height, &bits, &isHDR, true);
            uint8_t* xn = Index::LoadImageFromFile(xneg, &width, &height, &bits, &isHDR, true);
            uint8_t* yp = Index::LoadImageFromFile(ypos, &width, &height, &bits, &isHDR, true);
            uint8_t* yn = Index::LoadImageFromFile(yneg, &width, &height, &bits, &isHDR, true);
            uint8_t* zp = Index::LoadImageFromFile(zpos, &width, &height, &bits, &isHDR, true);
            uint8_t* zn = Index::LoadImageFromFile(zneg, &width, &height, &bits, &isHDR, true);
        ...
        uint32_t GLTextureCube::LoadFromVCross(uint32_t mips)
        {
            ...
            {
                bool isHDR          = false;
                uint8_t* data       = Index::LoadImageFromFile(m_Files[m], &srcWidth, &srcHeight, &bits, &isHDR, !m_LoadOptions.flipY);
                m_Parameters.format = BitsToFormat(bits);
