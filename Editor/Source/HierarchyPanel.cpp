...
#include <Index/Scene/SceneGraph.h>
...
namespace Index
{
    ...
    {
        m_Name       = ICON_MDI_TEXT "Hierarchy###hierarchy";
    ...
    void HierarchyPanel::DrawNode(entt::entity node, entt::registry& registry)
    {
        ...
        static const char* defaultName     = "Game Object";
        ...
        if(show)
        {
            ...
            bool nodeOpen = ImGui::TreeNodeEx((void*)(intptr_t)entt::to_integral(node), nodeFlags, "%s", icon.c_str());
            {
                ...
                {
                    bool ctrlDown = Input::Get().GetKeyHeld(Index::InputCode::Key::LeftControl) || Input::Get().GetKeyHeld(Index::InputCode::Key::RightControl) || Input::Get().GetKeyHeld(Index::InputCode::Key::LeftSuper);
                    if(!ctrlDown)
            ...
            if(ImGui::BeginPopupContextItem(name))
            {
                ...
                if (ImGui::BeginMenu("2D Object"))
                {
                    ...
                    {
                        auto entity = scene->CreateEntity("New Sprite");
                        entity.AddComponent<Graphics::ModelComponent>(Graphics::PrimitiveType::Plane);
                    }
                    ImGui::EndMenu();
                }
            ...
            if(payload != NULL && payload->IsDataType("Drag_Entity"))
            {
                ...
                INDEX_ASSERT(payload->DataSize == sizeof(entt::entity*), "Error ImGUI drag entity");
    ...
    void HierarchyPanel::OnImGui()
    {
        ...
        m_SelectUp   = Input::Get().GetKeyPressed(Index::InputCode::Key::Up);
        m_SelectDown = Input::Get().GetKeyPressed(Index::InputCode::Key::Down);
        ...
        ImGui::Begin(m_Name.c_str(), &m_Active, flags);
        {
            ...
            auto AddEntity = [scene]()
            {
                ...
                if (ImGui::Selectable("Empty Game Object"))
                {
                    scene->CreateEntity();
                }
                ...
                {
                    auto entity = scene->CreateEntity("Lua Script");
