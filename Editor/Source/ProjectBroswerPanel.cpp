...
#include <Index/Core/VFS.h>
#include <Index/Core/OS/Window.h>

#include <Index/ImGui/IconsMaterialDesignIcons.h>
#include <imgui/imgui.h>
#include <imgui/imgui_internal.h>
#include <Index/Embedded/browserFile.inl>
#include <Index/Embedded/browserFolder.inl>
...
namespace Index
{
    ...
    static const std::unordered_map<FileType, const char*> s_FileTypesToString = {
        ...
        { FileType::Scene, "Index Scene" },
        { FileType::Prefab, "Prefab" },
        { FileType::Script, "Script" },
        { FileType::ScriptCS, "CSharp Script" },
    ...
    static const std::unordered_map<std::string, FileType> s_FileTypes = {
        { ".Index", FileType::Scene },

        { ".prefab", FileType::Prefab },

        { ".cs", FileType::ScriptCS },
        { ".lua", FileType::Script },
    ...
    static const std::unordered_map<FileType, ImVec4> s_TypeColors = {
        ...
        { FileType::ScriptCS, { 0.00f, 0.80f, 0.00f, 1.00f } },
    ...
    static const std::unordered_map<FileType, const char*> s_FileTypesToIcon = {
		...
		{ FileType::Scene, ICON_MDI_MAP},
        { FileType::Prefab, ICON_MDI_CUBE },
        { FileType::Script, ICON_MDI_LANGUAGE_LUA },
        { FileType::ScriptCS, ICON_MDI_LANGUAGE_CSHARP},
...
#ifdef INDEX_PLATFORM_WINDOWS
    std::string ProjectBroswerPanel::m_Delimiter = "\\";
#else
    std::string ProjectBroswerPanel::m_Delimiter = "/";
#endif
    ProjectBroswerPanel::ProjectBroswerPanel()
    {
        INDEX_PROFILE_FUNCTION();
		m_Name = ICON_MDI_FOLDER "Project###Project";
        m_SimpleName = "Project";
    ...
    void ProjectBroswerPanel::ChangeDirectory(SharedPtr<DirectoryInformation>& directory)
    {
        if(!directory)
            return;
    ...
    void ProjectBroswerPanel::RemoveDirectory(SharedPtr<DirectoryInformation>& directory, bool removeFromParent)
    {
    ...
    std::string ProjectBroswerPanel::ProcessDirectory(const std::filesystem::path& directoryPath, const SharedPtr<DirectoryInformation>& parent)
    {
        ...
        if(std::filesystem::is_directory(directoryPath))
        {
            ...
            {
                if(!m_ShowHiddenFiles && Index::StringUtilities::IsHiddenFile(entry.path().string()))
                {
                    continue;
                }
                {
    ...
    void ProjectBroswerPanel::DrawFolder(const SharedPtr<DirectoryInformation>& dirInfo, bool defaultOpen)
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void ProjectBroswerPanel::OnImGui()
    {
        INDEX_PROFILE_FUNCTION();
        ...
        if(ImGui::Begin(m_Name.c_str(), &m_Active))
        {
            ...
            if(!vertical)
            {
                ImGui::BeginColumns("ProjectBroswerPanelColumns", 2, 0);
                if(!Init)
                {
            ...
            if(ImGui::BeginDragDropTarget())
            {
                ...
                if(data)
                {
                    ...
                    {
                        INDEX_LOG_INFO("Moved File: " + file + " to " + m_MovePath);
                    }
                    m_IsDragging = false;
                }
                    ...
                    if(ImGui::BeginTable("BodyTable", columnCount, flags))
                    {
                        ...
                        if(m_IsInListView)
                        {
                            for(int i = 0; i < m_CurrentDir->Children.size(); i++)
                            {
                                ...
                                {
                                    if(!m_ShowHiddenFiles && Index::StringUtilities::IsHiddenFile(m_CurrentDir->Children[i]->FilePath.filename().string()))
                                    {
                                        continue;
                                    }
                        ...
                        if(ImGui::BeginPopupContextWindow("AssetPanelHierarchyContextWindow", ImGuiPopupFlags_MouseButtonRight | ImGuiPopupFlags_NoOpenOverItems))
                        {
                            ...
                            if (ImGui::Selectable("Open Location"))
                            {
                                ...
                                Index::OS::Instance()->OpenFileLocation(fullPath);
                            }
            ...
            if(ImGui::BeginDragDropTarget())
            {
                ...
                if(data)
                {
                    ...
                    {
                        INDEX_LOG_INFO("Moved File: " + a + " to " + m_MovePath);
                    }
                    m_IsDragging = false;
                }
                ImGui::EndDragDropTarget();
    ...
    void ProjectBroswerPanel::RenderBreadCrumbs()
    {
        INDEX_PROFILE_FUNCTION();
    ...
    bool ProjectBroswerPanel::RenderFile(int dirIndex, bool folder, int shownIndex, bool gridView)
    {
        INDEX_PROFILE_FUNCTION();
        ...
        if(gridView)
        {
            ...
            if(CurrentEnty->IsFile)
            {
                if(CurrentEnty->Type == FileType::Texture)
                {
                    ...
                    {
                        textureCreated         = true;
                        CurrentEnty->Thumbnail = m_TextureLibrary.GetResource(m_BasePath + "/" + CurrentEnty->FilePath.string());
            ...
            if(ImGui::BeginPopupContextItem())
            {
                ...
                if (ImGui::BeginMenu("Create"))
                {
                    ...
                    {
                        std::filesystem::create_directory(std::filesystem::path(m_CurrentDir->FilePath.string() + "/New Folder"));
                        Refresh();
                    }
                    ImGui::EndMenu();
                }
                ...
                {
                    auto base = std::filesystem::path(m_BasePath);
                    base.make_preferred();
                    auto fullPath = base / m_CurrentDir->Children[dirIndex]->FilePath;
                ...
                if (ImGui::MenuItem("Open External"))
                {
                    ...
                    Index::OS::Instance()->OpenFileExternal(fullPath);
                }
            ...
            if(ImGui::IsItemHovered() && m_CurrentDir->Children[dirIndex]->Thumbnail)
            {
                ...
                ImGui::TextUnformatted((m_BasePath + "/" + m_CurrentDir->Children[dirIndex]->FilePath.filename().string()).c_str());
                ...
                ImGuiUtilities::Tooltip((m_BasePath + "/" + m_CurrentDir->Children[dirIndex]->FilePath.filename().string()).c_str());
    ...
    void ProjectBroswerPanel::RenderBottom()
    {
        INDEX_PROFILE_FUNCTION();
    ...
    std::vector<std::string> ProjectBroswerPanel::SearchFiles(const std::string& query)
    {
        return std::vector<std::string>();
    }

    bool ProjectBroswerPanel::MoveFile(const std::string& filePath, const std::string& movePath)
    {
        INDEX_PROFILE_FUNCTION();
    ...
    std::string ProjectBroswerPanel::StripExtras(const std::string& filename)
    {
        INDEX_PROFILE_FUNCTION();
    ...
    void ProjectBroswerPanel::OnNewProject()
    {
        Refresh();
    }

    void ProjectBroswerPanel::Refresh()
    {
