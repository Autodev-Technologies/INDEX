...
namespace Index
{
    ...
    {
		m_Name = ICON_MDI_SETTINGS "Scene Settings###scene settings";
            ...
            Index::ImGuiUtilities::PushID();
            {
...

                {
                    ...
                    if(!ImGui::IsItemActive() && m_NameUpdated)
                    {
                        ...
                        if(VFS::Get().ResolvePhysicalPath("//Scenes/" + m_CurrentScene->GetSceneName() + ".Index", scenePath))
                        {
                            m_CurrentScene->SetName(sceneName);
                            // m_CurrentScene->Serialise(m_Editor->GetProjectSettings().m_ProjectRoot + "Assets/Scenes/");

                            std::filesystem::rename(scenePath, m_Editor->GetProjectSettings().m_ProjectRoot + "Assets/Scenes/" + m_CurrentScene->GetSceneName() + ".Index");
                        }
                        else
                            m_CurrentScene->SetName(sceneName);
                ...
                {
                    ImGui::Columns(2);
                    auto physicsSystem = Application::Get().GetSystem<IndexPhysicsEngine>();
